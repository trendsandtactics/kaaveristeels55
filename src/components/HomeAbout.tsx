  ...
  return (
    <section className="relative w-full min-h-screen flex flex-col lg:flex-row items-stretch bg-slate-50/50 overflow-hidden">
      ...
      <motion.div 
        ...
        className="w-full lg:w-1/2 flex flex-col items-start px-6 sm:px-12 md:px-16 lg:px-20 py-16 lg:py-24 bg-white relative z-10 lg:shadow-[-20px_0_40px_rgba(0,0,0,0.03)]"
      >
        <div className="absolute top-0 right-0 w-64 h-64 bg-slate-100/40 rounded-full blur-3xl pointer-events-none -z-10" />

        <div className="w-full max-w-4xl flex flex-col justify-between h-full flex-grow gap-12 lg:gap-8">
          
          <motion.div variants={itemVariants} className="flex flex-col items-start">
            <span className="font-sans uppercase tracking-[0.25em] text-red-600 text-lg lg:text-xl font-black block mb-4">
              ABOUT KAAVERI STEELS
            </span>
            <div className="w-16 h-[5px] bg-red-600 rounded-full" />
          </motion.div>

          <motion.div variants={itemVariants} className="space-y-8 flex-grow flex flex-col justify-center">
            <h2 className="font-serif font-black text-5xl lg:text-6xl xl:text-7xl text-slate-900 tracking-tight leading-[1.1]">
            ...
            <div className="space-y-6">
              <p className="font-sans text-slate-600 font-normal leading-relaxed text-xl lg:text-2xl">
          ...
          <motion.div 
            ...
            className="relative border-l-4 border-red-600 pl-6 sm:pl-10 py-5 sm:py-8 lg:py-6 text-left bg-gradient-to-r from-slate-50 to-transparent rounded-r-xl border-y border-r border-slate-100/50 my-6"
          >
            <p className="font-sans italic text-slate-700 text-2xl lg:text-3xl font-medium leading-relaxed">
          ...
          <motion.div 
            variants={itemVariants}
            className="flex flex-col sm:flex-row items-center justify-start gap-4 sm:gap-6 lg:gap-6 pt-4 w-full"
          >
            <Link 
              href="/products" 
              className="group font-sans w-full sm:w-auto px-10 py-5 lg:px-12 lg:py-6 bg-gradient-to-r from-red-600 to-red-700 hover:from-red-700 hover:to-red-800 text-white font-bold text-lg lg:text-xl tracking-wider transition-all duration-300 shadow-md hover:shadow-xl hover:shadow-red-600/20 flex items-center justify-center gap-3 uppercase whitespace-nowrap rounded-md"
            >
              Explore Products 
            ...
            <Link 
              ...
              className="font-sans w-full sm:w-auto px-10 py-5 lg:px-12 lg:py-6 border-2 border-slate-200 text-slate-700 hover:text-slate-900 hover:bg-slate-50 hover:border-slate-400 font-bold text-lg lg:text-xl tracking-wider transition-all duration-300 flex items-center justify-center gap-3 uppercase whitespace-nowrap rounded-md"
            >
